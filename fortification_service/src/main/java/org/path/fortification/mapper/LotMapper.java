...
public class LotMapper implements Mappable<LotResponseDto, LotRequestDto, Lot>, ListMappable<LotListResponseDTO, LotRequestDto, Lot> {
    ...
    public LotResponseDto toDto(Lot entity) {
        ...
        if(dto.getDateOfExpiry()==null && dto.getDateOfManufacture()==null){
            Set<LotResponseDto> sourceLots = entity.getTargetBatchMapping()
                    .stream()
            ...
            BatchResponseDto batchResponseDto = sourceLots.stream().filter(sl->sl.getBatch()!=null).map(sourceBatch -> sourceBatch.getBatch())
                    .findFirst()
                    .orElse(null);
            if(batchResponseDto!=null) {
                dto.setDateOfExpiry(batchResponseDto.getDateOfExpiry());
                dto.setDateOfManufacture(batchResponseDto.getDateOfManufacture());
            }
        }
    ...
    public LotListResponseDTO toListDto(Lot entity) {
        ...
        if(entity.getBatch() != null) {
            ...
            BatchResponseDto batchResponseDto = sourceLots.stream().filter(sl->sl.getBatch()!=null).map(sourceBatch -> sourceBatch.getBatch())
                    .findFirst()
                    .orElse(null);
            assert batchResponseDto!=null;
            lotListResponseDTO.setDateOfExpiry(batchResponseDto.getDateOfExpiry());
            lotListResponseDTO.setDateOfManufacture(batchResponseDto.getDateOfManufacture());
        }
