...
public class UserDao extends BaseDao<User> {
    ...
    public User findByName(String userName) {
        try {
            return em.createQuery("FROM User u where u.userName like :userName", User.class)
    ...
    private void searchParams(TypedQuery<?> query, SearchListRequest searchRequest) {
        ...
        if (map.get("search") != null && !Objects.equals(map.get("search"), "")) {
            query.setParameter("search", "%" + EncryptionHelper.encrypt((String) map.get("search")).split(":")[1] + "%");
        }
        try {
