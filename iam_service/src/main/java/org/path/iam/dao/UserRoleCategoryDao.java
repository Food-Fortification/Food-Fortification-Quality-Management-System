...
public class UserRoleCategoryDao extends BaseDao<UserRoleCategory> {
    ...
    public List<UserRoleCategory> findAllByUserId(Long userId){
        try {
            return em.createQuery("from UserRoleCategory urc where urc.user.id = :userId and urc.isDeleted is false", UserRoleCategory.class)
                    .setParameter("userId",userId)
                    .getResultList();
        } catch (NoResultException e){
            return new ArrayList<>();
