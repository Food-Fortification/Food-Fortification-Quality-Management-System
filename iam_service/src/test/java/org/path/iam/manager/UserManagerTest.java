...
public class UserManagerTest {
    ...
    public void testFindByName() {
        ...
        // Mock static method EncryptionHelper.encrypt
        try (MockedStatic<EncryptionHelper> mockedStatic = mockStatic(EncryptionHelper.class)) {
            mockedStatic.when(() -> EncryptionHelper.encrypt(userName)).thenReturn(encryptedUserName);

            // Mock behavior of UserDao
            when(userDao.findByName(encryptedUserName)).thenReturn(expectedUser);
            ...
            // Verify that EncryptionHelper.encrypt was called with the correct argument
            mockedStatic.verify(() -> EncryptionHelper.encrypt(userName), times(1));

            verify(userDao, times(1)).findByName(encryptedUserName);
        }
    }
